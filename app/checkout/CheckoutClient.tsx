          ...
          if (response.status === 'successful' || response.status === 'completed') {
            try {
              const verifyResponse = await fetch('/api/payments/flutterwave/verify', {
                ...
                body: JSON.stringify({ transactionId: String(response.transaction_id), txRef: newTxRef, amount: total, customer: formData, items: cart }),
              });
              ...
              localStorage.setItem('lastOrder', JSON.stringify({
                _id: orderData._id, orderId: orderData._id, orderNumber: orderData.orderNumber,
                customer: formData, items: cart.map(item => ({ id: item.id, name: item.name, price: item.price, quantity: item.quantity, size: item.size, color: item.color, image: item.image })),
                total, paymentMethod: 'Flutterwave',
              }));
              clearCart();
              setTimeout(() => {
        ...
        onSuccess: async (ref: any) => {
          try {
            const verifyResponse = await fetch('/api/payments/paystack/verify', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ reference: ref.reference, amount: total, customer: formData, items: cart }),
            });
            const verifyPayload = await verifyResponse.json();
            if (!verifyResponse.ok || !verifyPayload?.success) throw new Error(verifyPayload?.error || 'Payment verification failed.');
            const orderData = verifyPayload.data;
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input type="text" name="country" value={formData.country} onChange={handleInputChange} placeholder="Nigeria" className={inputClass} />
                  </div>
                </div>
                  ...
                  {!isFlutterwaveKeyValid && !isPaystackKeyValid && (
                    <div className="mb-5 p-4 bg-red-950/50 border border-red-500/30">
                      <p className="text-[10px] font-mono text-red-300/80">
                        Payment gateway not configured. Set NEXT_PUBLIC_FLUTTERWAVE_PUBLIC_KEY and NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY in your environment variables.
                      </p>
                    </div>
                  )}

                  <div className="space-y-3">
                    <label className={`flex items-center gap-4 p-4 border cursor-pointer transition-colors ${paymentMethod === 'flutterwave' ? 'border-zibara-cream/40 bg-zibara-cream/5' : 'border-zibara-cream/10 hover:border-zibara-cream/25'} ${!isFlutterwaveKeyValid ? 'opacity-40 cursor-not-allowed' : ''}`}>
                      <input
                        type="radio"
                        name="paymentMethod"
                        value="flutterwave"
                      ...
                      />
                      <span className="text-[11px] font-mono text-zibara-cream/80">Flutterwave — Card, Bank Transfer, USSD</span>
                    </label>

                    <label className={`flex items-center gap-4 p-4 border cursor-pointer transition-colors ${paymentMethod === 'paystack' ? 'border-zibara-cream/40 bg-zibara-cream/5' : 'border-zibara-cream/10 hover:border-zibara-cream/25'} ${!isPaystackKeyValid ? 'opacity-40 cursor-not-allowed' : ''}`}>
                      <input
                        type="radio"
                        name="paymentMethod"
                        value="paystack"
                        ...
                        className="w-4 h-4 accent-zibara-cream"
                      />
                      <span className="text-[11px] font-mono text-zibara-cream/80">Paystack — Card, Bank Transfer</span>
                    </label>
                  </div>
                </div>
              </div>
            ...
            <div>
              <div className="lg:sticky lg:top-32">
                ...
                <div className="flex justify-between py-5">
                  <span className="text-[11px] font-mono text-zibara-cream/70 uppercase tracking-wider">Total</span>
                  <span className="text-base font-mono text-zibara-cream">{formatPrice(totalInUSD, 'USD')}</span>
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 border border-zibara-cream/35 text-[10px] uppercase tracking-[0.4em] font-mono text-zibara-cream/80 hover:bg-zibara-cream hover:text-zibara-black hover:border-zibara-cream transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed"
                >
