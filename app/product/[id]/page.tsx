...
export default function ProductDetailPage() {
    ...
    if (product.sizes?.length && !selectedSize) {
      toast.error("Please select a size", {
        style: {
          background: "#0a0806",
          color: "#EFEFC9",
          border: "1px solid rgba(239,239,201,0.1)",
        },
      });
      return;
    }
  ...
  if (productsLoading)
    return <BrandLoader label="Piece" sublabel="ZIBARASTUDIO" tone="deep" />;

  if (!product) {
    return (
      <div className="min-h-screen bg-zibara-black flex flex-col items-center justify-center gap-4 pt-24">
        <p className="text-[11px] font-mono text-zibara-cream/70 tracking-widest uppercase">
          Piece not found
        </p>
        <Link
          href="/shop"
          className="text-[10px] font-mono text-zibara-cream/60 hover:text-zibara-cream transition-colors tracking-widest uppercase border-b border-current pb-0.5"
        >
  ...
  return (
    <div className="min-h-screen bg-zibara-black text-zibara-cream pt-24 md:pt-28">
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-8 py-5">
        <div className="flex items-center gap-2 text-[10px] font-mono text-zibara-cream/75 tracking-widest uppercase">
          <Link href="/" className="hover:text-zibara-cream transition-colors">
            Home
          </Link>
          <span>/</span>
          <Link
            href="/shop"
            className="hover:text-zibara-cream transition-colors"
          >
            Shop
          </Link>
          <span>/</span>
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-8 py-8 md:py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 lg:gap-24">
          ...
          <div className="md:sticky md:top-28 md:self-start space-y-8">
            ...
            {/* Price */}
            <p className="text-2xl font-mono text-zibara-gold">
            ...
            <div className="space-y-3">
              ...
              <div className="inline-flex items-center border border-zibara-cream/25">
                ...
                <span className="w-10 text-center text-[11px] font-mono text-zibara-cream">
                  {quantity}
                </span>
                <button
            ...
            <button
              ...
              className={`w-full py-4 text-[10px] tracking-[0.4em] uppercase font-mono transition-all duration-300 border ${
                !product.inStock
                  ? "border-zibara-cream/10 text-zibara-cream/30 cursor-not-allowed"
                  : addedToCart
                    ? "bg-zibara-cream border-zibara-cream text-zibara-black"
                    : "bg-zibara-crimson border-zibara-crimson text-zibara-cream hover:bg-zibara-cream hover:text-zibara-black hover:border-zibara-cream"
              }`}
            >
              {!product.inStock
                ? "Sold Out"
            ...
            {product.care?.length > 0 && (
              <div className="border-t border-zibara-cream/10">
                ...
                >
                  ...
                  <ChevronDown
                    size={14}
                    className={`transition-transform duration-300 ${careOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {careOpen && (
                  <ul className="pb-4 space-y-1">
                    {product.care.map((c: string, i: number) => (
                      <li
                        key={i}
                        className="text-[10px] font-mono text-zibara-cream/70"
                      >
                        — {c}
                      </li>
                    ))}
      ...
      {relatedProducts.length > 0 && (
        <section className="max-w-[1400px] mx-auto px-6 md:px-8 py-16 border-t border-zibara-cream/10">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link
              href="/shop"
              className="text-[10px] tracking-widest font-mono text-zibara-cream/55 hover:text-zibara-cream transition-colors uppercase"
            >
              View all →
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {relatedProducts.map((p, i) => (
              <Link key={p._id} href={`/product/${p._id}`} className="group">
                ...
                <p className="text-[10px] font-mono text-zibara-cream">
                  {formatPrice(p.price)}
                </p>
              </Link>
            ))}
