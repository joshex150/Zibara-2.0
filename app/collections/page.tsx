...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    fetch('/api/collections')
      .then(r => r.json())
      .then(data => { if (data.success) setCollections(data.data); })
      .catch(console.error)
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-8">
        ...
        {featured.length > 0 && (
          <div className="space-y-2 mb-4">
            {featured.map((col, i) => (
              <Link key={col._id} href={`/collections/${col.slug}`} className="group block relative overflow-hidden">
        ...
        {remaining.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2 mb-24">
            {remaining.map((col) => (
              <Link key={col._id} href={`/collections/${col.slug}`} className="group relative overflow-hidden aspect-[4/3]">
                {col.coverImage ? (
                  <ZibaraPlaceholder
                    label={col.name}
                    sublabel={`${col.season} ${col.year}`}
