        ...
        {filteredContents.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filteredContents.map((content) => (
              <div key={content._id} className="bg-[#f5d5e5] rounded-lg shadow-md p-4 md:p-6 flex flex-col">
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h3 className="font-bold text-gray-900 text-xs md:text-sm break-all">{content.key}</h3>
                    <span className="text-[10px] md:text-xs bg-[#8b2b4d] text-white px-1.5 md:px-2 py-0.5 md:py-1 rounded shrink-0">
                      {content.type}
                    </span>
                  </div>
                  <p className="text-[10px] md:text-xs text-gray-600 mb-2 md:mb-3">{content.description}</p>
                ...
                <button
                  ...
                  className="w-full bg-[#8b2b4d] text-white px-4 py-2 rounded text-xs md:text-sm font-semibold hover:bg-[#6d1f3a] transition-colors mt-3 md:mt-4"
                >
                  Edit
                </button>
              </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 md:p-4">
            <div className="bg-[#f5d5e5] rounded-lg max-w-2xl w-full max-h-[95vh] overflow-y-auto">
              <div className="p-4 md:p-6 border-b">
                ...
                <p className="text-xs md:text-sm text-gray-600 mt-1">{editingContent.description}</p>
              </div>
              ...
              <div className="p-4 md:p-6 space-y-3 md:space-y-4">
                ...
                <div className="bg-white/50 rounded-lg p-3 md:p-4 space-y-1.5 md:space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] md:text-xs font-semibold text-gray-500 uppercase">Key:</span>
                    <span className="text-xs md:text-sm text-gray-900 break-all">{editingContent.key}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] md:text-xs font-semibold text-gray-500 uppercase">Section:</span>
                    <span className="text-xs md:text-sm text-gray-900 capitalize">{editingContent.section}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] md:text-xs font-semibold text-gray-500 uppercase">Type:</span>
                    <span className="text-xs md:text-sm text-gray-900 capitalize">{editingContent.type}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {editingContent.type === 'richtext' || editingContent.type === 'text' ? (
                    <textarea
                      ...
                      className="w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8b2b4d] focus:border-transparent min-h-[120px] md:min-h-[150px] text-sm bg-white"
                        ...
                        {({
                          ...
                          <div className="space-y-3">
                              ...
                              <div className="relative aspect-video bg-gray-100 rounded-lg overflow-hidden border-2 border-gray-300">
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                  <button
                                    ...
                                    className="px-4 py-2 bg-white text-gray-700 rounded text-sm font-semibold hover:bg-gray-100"
                                  >
                                    Update
                                  </button>
                                  <button
                            ...
                            <input
                              ...
                              className="w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8b2b4d] focus:border-transparent text-sm bg-white"
              ...
              <div className="p-4 md:p-6 border-t flex flex-col sm:flex-row justify-end gap-2 md:gap-4">
                <button
                  ...
                  className="px-4 md:px-6 py-2 border border-gray-300 rounded-lg font-semibold hover:bg-gray-50 transition-colors text-sm order-2 sm:order-1"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={saving}
                  className="flex items-center justify-center gap-2 bg-[#8b2b4d] text-white px-4 md:px-6 py-2 rounded-lg font-semibold hover:bg-[#6d1f3a] transition-colors disabled:opacity-50 text-sm order-1 sm:order-2"
                >
