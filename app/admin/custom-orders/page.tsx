...
export default function AdminCustomOrdersPage() {
  ...
  return (
    <div className="min-h-screen bg-[#EBB0C9] flex flex-col scroll-mt-32">
      <div className="max-w-7xl mx-auto px-4 py-8 w-full flex-1 flex flex-col">
        ...
        {filteredOrders.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center py-12 md:py-16">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-[#8b2b4d] rounded-full flex items-center justify-center mb-4 md:mb-6">
              <Palette className="w-8 h-8 md:w-10 md:h-10 text-white" />
            </div>
            <h3 className="text-lg md:text-xl font-bold text-[#8b2b4d] mb-2">No Custom Orders</h3>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-bold text-[#8b2b4d] text-sm md:text-base">
                    ...
                    {order.colors.length > 0 && (
                      <div className="flex -space-x-1">
                        {order.colors.slice(0, 3).map((color, idx) => (
                          <span 
                            key={idx}
                            className="w-5 h-5 rounded-full border-2 border-white text-[8px] flex items-center justify-center bg-[#8b2b4d] text-white"
                            title={color}
                          >
                            {color[0]}
                          </span>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between">
              <h2 className="font-bold text-lg text-[#8b2b4d]">
            ...
            <div className="p-4 md:p-6 space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <select
                  ...
                  disabled={saving}
                  className="px-3 py-2 border border-[#8b2b4d]/20 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#8b2b4d] focus:border-transparent bg-[#f5d5e5] text-[#8b2b4d]"
                >
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <a href={`mailto:${selectedOrder.email}`} className="flex items-center gap-2 text-[#8b2b4d] hover:underline">
                    <Mail size={14} /> {selectedOrder.email}
                  </a>
                  <a href={`tel:${selectedOrder.phone}`} className="flex items-center gap-2 text-[#8b2b4d] hover:underline">
                ...
                <div className="space-y-3 text-sm">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {selectedOrder.colors.map((color, idx) => (
                          <span key={idx} className="px-2 py-0.5 bg-[#f5d5e5] rounded text-xs">
                            {color}
                          </span>
                        ))}
                      </div>
                  ...
                  {selectedOrder.referenceImages && selectedOrder.referenceImages.length > 0 && (
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">Click on images to view full size</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    {selectedOrder.measurements.bust && (
                      <div className="bg-gray-50 p-2 rounded">
                        <span className="text-gray-500 text-xs">Bust</span>
                        <p className="font-medium">{selectedOrder.measurements.bust}</p>
                      </div>
                    )}
                    {selectedOrder.measurements.waist && (
                      <div className="bg-gray-50 p-2 rounded">
                        <span className="text-gray-500 text-xs">Waist</span>
              ...
              {selectedOrder.additionalNotes && (
                <div>
                  ...
                  <p className="text-sm bg-gray-50 p-3 rounded">{selectedOrder.additionalNotes}</p>
                </div>
              )}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#8b2b4d] resize-none"
                />
                <button
                  onClick={saveAdminNotes}
                  disabled={saving}
                  className="mt-2 px-4 py-2 bg-[#8b2b4d] text-white text-sm rounded-lg hover:bg-[#6d1f3a] transition-colors disabled:opacity-50"
                >
